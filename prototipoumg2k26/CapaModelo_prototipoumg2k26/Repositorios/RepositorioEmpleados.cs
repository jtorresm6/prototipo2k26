...
namespace CapaModelo_prototipoumg2k26.Repositorios
{
    public class RepositorioEmpleados : RepositorioMaestro, IRepositorioEmpleados
    {
        ...
        {
            selectAll = "SELECT * FROM tbl_empleados";
            insert = "INSERT INTO tbl_empleados (dpi_emp, nit_emp, nombre_emp, apellido_emp, fecha_nacimiento, direccion_emp, fecha_contratacion, estado_emp, id_puesto) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            update = "UPDATE tbl_empleados SET dpi_emp=?, nit_emp=?, nombre_emp=?, apellido_emp=?, fecha_nacimiento=?, direccion_emp=?, fecha_contratacion=?, estado_emp=?, id_puesto=? WHERE id_empleado=?";
            delete = "DELETE FROM tbl_empleados WHERE id_empleado=?";
        }
        ...
        public IEnumerable<Empleados> GetAll()
        {
            ...
            foreach (DataRow row in TblTabla.Rows)
            {
                ...
                listaEmpleados.Add(empleado); // Agregado para que retorne los registros en la lista
            }
            TblTabla.Clear();
            TblTabla = null;
